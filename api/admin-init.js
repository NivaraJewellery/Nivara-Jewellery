...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    const existingCollections = await sql`select count(*)::int as count from collections`;

    if (existingCollections[0].count === 0) {
      const collectionNames = [...new Set(seedProducts.map(product => product.category || 'Necklace'))];
    ...
    if (existing[0].count === 0) {
      for (const product of seedProducts) {
        const collections = await sql`select id from collections where name = ${product.category} limit 1`;
        await sql`
          insert into products (name, category, type, price, stock, description, code, care, image, collection_id, active)
          values (${product.name}, ${product.category}, ${product.type}, ${product.price}, ${product.stock}, ${product.description}, ${product.code}, ${product.care}, ${product.image}, ${collections[0]?.id || null}, true)
        `;
      }
    }
    ...
    return send(response, 200, {
      ok: true,
      seeded: existing[0].count === 0,
      productCount: existing[0].count === 0 ? seedProducts.length : existing[0].count
    });
  } catch (error) {
