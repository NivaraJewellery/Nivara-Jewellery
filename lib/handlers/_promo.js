const PROMO_CODE = String(process.env.LAUNCH_PROMO_CODE || 'GANESHA5').trim().toUpperCase();
const PROMO_PERCENT = Number(process.env.LAUNCH_PROMO_PERCENT || 5);
const PROMO_START = new Date(process.env.LAUNCH_PROMO_START || '2026-09-12T04:55:00.000Z'); // 12 Sep 2026, 10:25 AM IST
const PROMO_END = new Date(process.env.LAUNCH_PROMO_END || '2026-09-14T18:30:00.000Z'); // exclusive: 15 Sep 2026, 12:00 AM IST
...
function getPromoMessage(state = getPromoState()) {
  ...
  if (state === 'ended') return 'This launch offer ended on 14 Sep at 11:59 PM IST.';
...
async function ensurePromoTable(sql) {
  ...
  `;
  await sql`alter table promo_redemptions add column if not exists email text`;
  await sql`create index if not exists idx_promo_redemptions_order on promo_redemptions(razorpay_order_id)`;
  await sql`create unique index if not exists uq_promo_redemptions_code_email on promo_redemptions(promo_code, lower(email)) where email is not null`;
}
...
async function getRegisteredPromoCustomer(sql, customerInput) {
  ...
  if (customer.email_verified === false) {
    const error = new Error('Please verify your email address before using this promo code.');
    error.statusCode = 400;
    throw error;
  }
  const phone = normalizePhone(customer.phone);
  if (phone.length < 10) {
    const error = new Error('A valid registered mobile number is required to use this promo code.');
    error.statusCode = 400;
    throw error;
  }

  return { ...customer, normalizedPhone: phone, normalizedEmail: String(customer.email || '').trim().toLowerCase() };
}
...
async function checkPromoEligibility(sql, customer, code) {
  ...
  const matches = await sql`
    select id, customer_id, phone, email, status, reserved_until, razorpay_order_id
    from promo_redemptions
    where promo_code = ${PROMO_CODE}
      and (customer_id = ${customer.id} or lower(email) = ${customer.normalizedEmail} or phone = ${customer.normalizedPhone})
    order by id desc
    limit 1
  `;
  ...
  if (matches.length) {
    ...
    if (
      ...
      new Date(existing.reserved_until) <= new Date()
    ) {
      await sql`
    ...
    if (existing.status === 'redeemed') {
      const error = new Error('This launch promo has already been used for this account, verified email, or registered mobile number.');
...
async function reservePromo(transaction, customer, code, razorpayOrderId) {
  ...
  const matches = await transaction`
    select id, customer_id, phone, email, status, reserved_until
    from promo_redemptions
    where promo_code = ${PROMO_CODE}
      and (customer_id = ${customer.id} or lower(email) = ${customer.normalizedEmail} or phone = ${customer.normalizedPhone})
    order by id desc
  ...
  if (matches.length) {
    ...
    await transaction`
      insert into promo_redemptions (promo_code, customer_id, phone, email, razorpay_order_id, status, reserved_until)
      values (${PROMO_CODE}, ${customer.id}, ${customer.normalizedPhone}, ${customer.normalizedEmail}, ${razorpayOrderId}, 'reserved', now() + (${RESERVATION_MINUTES} * interval '1 minute'))
    `;
  }
